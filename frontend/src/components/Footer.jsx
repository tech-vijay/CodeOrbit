      ...
      <div className="mx-auto max-w-7xl px-5 py-16 sm:px-8">
        <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <ul className="mt-4 space-y-3 text-sm">{services.slice(0, 6).map((s) => (<li key={s.title}><Link to={`/services/${s.slug}`} className="transition-colors hover:text-cyan-400">{s.title}</Link></li>))}</ul>
          </div>
          <div>
            <h4 className="text-sm font-semibold uppercase tracking-wider text-white">Contact</h4>
            <ul className="mt-4 space-y-3 text-sm">
              <li className="flex items-start gap-3"><Mail className="mt-0.5 h-4 w-4 flex-shrink-0 text-cyan-400" /><span className="break-all">codeorbitsoftwaresolutions@gmail.com</span></li>
